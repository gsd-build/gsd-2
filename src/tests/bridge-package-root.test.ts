...
test('bridge-service resolveBridgeRuntimeConfig falls back to env var when default is null', () => {
  const source = readFileSync(join(process.cwd(), 'src', 'web', 'bridge-service.ts'), 'utf-8')
  assert.ok(
    source.includes('env.GSD_WEB_PACKAGE_ROOT || DEFAULT_PACKAGE_ROOT || process.cwd()'),
    'resolveBridgeRuntimeConfig must have triple-fallback: env var, safe default, cwd',
  )
})
