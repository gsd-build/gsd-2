...
test('diagnoseSounddeviceError: ModuleNotFoundError must return missing-module', () => {
	...
	assert.strictEqual(diagnoseSounddeviceError(stderr), "missing-module",
		"ModuleNotFoundError for sounddevice should be 'missing-module', not 'missing-portaudio'");
});

test("diagnoseSounddeviceError: 'No module named sounddevice' variant", () => {
	const stderr = "ImportError: No module named sounddevice";
	assert.strictEqual(diagnoseSounddeviceError(stderr), "missing-module",
		"'No module' substring should return missing-module");
});

test('diagnoseSounddeviceError: actual portaudio error', () => {
	const stderr = "OSError: PortAudio library not found";
	assert.strictEqual(diagnoseSounddeviceError(stderr), "missing-portaudio",
		"PortAudio library error should return missing-portaudio");
});

test('diagnoseSounddeviceError: lowercase portaudio error', () => {
...
test('diagnoseSounddeviceError: unrelated error', () => {
	const stderr = "SyntaxError: invalid syntax";
	assert.strictEqual(diagnoseSounddeviceError(stderr), "unknown",
		"unrelated error should return unknown");
});

test('diagnoseSounddeviceError: empty stderr', () => {
	assert.strictEqual(diagnoseSounddeviceError(""), "unknown",
		"empty stderr should return unknown");
});

// ── ensureVoiceVenv ──────────────────────────────────────────────────

test('ensureVoiceVenv: returns true when venv already exists', () => {
...
test('ensureVoiceVenv: creates venv when missing', () => {
	const notifications: string[] = [];
...
test('ensureVoiceVenv: returns false and notifies on failure', () => {
