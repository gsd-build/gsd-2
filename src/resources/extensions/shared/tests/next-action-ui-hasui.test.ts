...
describe("showNextAction ctx.hasUI guard (#5125 lockup root protection)", () => {
  ...
  it("returns 'not_yet' when UI mode is rpc even if ctx.hasUI is true", async () => {
    let customCalled = 0;
    let selectCalled = 0;
    ...
    assert.equal(result, "not_yet", "rpc-backed headless UI must not emit interactive prompts");
