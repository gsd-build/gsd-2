 ...
 */
export type WorktreeLifecycleTestOverrides =
...
function isSamePathPhysical(a: string, b: string): boolean {
	return (
		normalizeWorktreePathForCompare(a) === normalizeWorktreePathForCompare(b)
	);
}
...
export type ExitResult =
	...
	| {
			ok: false;
			reason: "merge-conflict" | "teardown-failed";
			cause?: unknown;
	  };
...
type WorktreeLifecyclePrimitiveOverrides = {
	...
	loadEffectiveGSDPreferences?: (
		basePath?: string,
	) => { preferences?: { git?: Record<string, unknown> } } | null | undefined;
};
...
function readLifecycleFile(deps: WorktreeLifecycleDeps, path: string): string {
	return (
		primitiveOverrides(deps).readFileSync?.(path, "utf-8") ??
...
function currentLifecycleBranch(
	...
	return (
		primitiveOverrides(deps).getCurrentBranch?.(basePath) ??
...
function autoCommitLifecycleBranch(
	...
	return (
		primitiveOverrides(deps).autoCommitCurrentBranch?.(
			basePath,
			unitType,
			unitId,
...
function lifecycleIsInAutoWorktree(
	...
	return (
		primitiveOverrides(deps).isInAutoWorktree?.(basePath) ??
...
function lifecycleAutoWorktreeBranch(
	...
	return (
		primitiveOverrides(deps).autoWorktreeBranch?.(milestoneId) ??
...
function lifecycleCreateAutoWorktree(
	...
	return (
		primitiveOverrides(deps).createAutoWorktree?.(basePath, milestoneId) ??
...
function lifecycleEnterAutoWorktree(
	...
	return (
		primitiveOverrides(deps).enterAutoWorktree?.(basePath, milestoneId) ??
...
function lifecycleGetIsolationMode(
	...
	return (
		primitiveOverrides(deps).getIsolationMode?.(basePath) ??
...
function lifecycleResolveMilestoneFile(
	...
	return (
		primitiveOverrides(deps).resolveMilestoneFile?.(
			basePath,
			milestoneId,
			fileType,
...
function rebuildGitService(s: AutoSession, deps: WorktreeLifecycleDeps): void {
...
function _mergeWorktreeModeImpl(
	...
	try {
		...
		const { synced } =
			deps.worktreeProjection.finalizeProjectionForMerge(finalScope);
		if (synced.length > 0) {
			debugLog("WorktreeLifecycle", {
				action: "mergeAndExit",
				milestoneId,
		...
		// #5306: Merge eligibility is determined by implementation commits on the
		// milestone branch, not by the git-tracked status of ROADMAP.md. The roadmap
		// content is best-effort for commit message generation — its absence must NOT
		// gate the merge.
		if (!roadmapPath) {
			const milestoneBranch = autoWorktreeBranch(milestoneId);
			let hasCommits = false;
			try {
				if (nativeBranchExists(originalBasePath, milestoneBranch)) {
					const mainBranch = nativeDetectMainBranch(originalBasePath);
					const commitsAhead = nativeCommitCountBetween(
						originalBasePath,
						mainBranch,
						milestoneBranch,
					);
					hasCommits = commitsAhead > 0;
				}
			} catch {
				// Rev-walk failure — fall through to legacy preserve-branch behavior
			}

			if (!hasCommits) {
				// No roadmap AND no commits — nothing to merge. Preserve branch (#1573).
				lifecycleTeardownAutoWorktree(deps, originalBasePath, milestoneId, {
					preserveBranch: true,
				});
				notify(
					`Exited worktree for ${milestoneId} (no roadmap found and no commits on branch — branch preserved for manual merge).`,
					"warning",
				);
				return {
					merged: false,
			...
			// Has commits but no roadmap — proceed with merge using empty roadmap content.
			// mergeMilestoneToMain will fall back to DB-sourced commit message (#5306).
			debugLog("WorktreeLifecycle", {
				action: "mergeAndExit",
				milestoneId,
				phase: "roadmap-missing-but-has-commits",
				note: `${milestoneBranch} has implementation commits — proceeding with merge despite missing roadmap`,
			});
			notify(
		...
		const roadmapContent = roadmapPath
			? readLifecycleFile(deps, roadmapPath)
			: "";
...
function _mergeBranchModeImpl(
	...
	try {
		...
		// #5306: Same commit-based eligibility check as worktree mode.
		if (!roadmapPath) {
			const milestoneBranch = lifecycleAutoWorktreeBranch(deps, milestoneId);
			let hasCommits = false;
			try {
				if (nativeBranchExists(worktreeBasePath, milestoneBranch)) {
					const mainBranch = nativeDetectMainBranch(worktreeBasePath);
					const commitsAhead = nativeCommitCountBetween(
						worktreeBasePath,
						mainBranch,
						milestoneBranch,
				...
				// Rev-walk failure — fall through to preserve-branch
			}
			...
			// Has commits but no roadmap — proceed with empty content.
			debugLog("WorktreeLifecycle", {
				action: "mergeAndExit",
				milestoneId,
				mode: "branch",
...
export function mergeMilestoneStandalone(
	...
	const targetCwd =
		mode === "worktree" || inWorktree ? worktreeBasePath : originalBasePath;
	if (targetCwd) {
		try {
...
export class WorktreeLifecycle {
	...
	adoptOrphanWorktree<T extends { merged: boolean }>(
		...
		try {
			const wtPathFn =
				primitiveOverrides(this.deps).getAutoWorktreePath ??
				getAutoWorktreePath;
