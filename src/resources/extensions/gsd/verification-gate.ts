...
export function isLikelyCommand(cmd: string): boolean {
  ...
  if (!effectiveFirstToken) return false;
  ...
  if (
    effectiveFirstToken.startsWith("/") ||
    effectiveFirstToken.startsWith("./") ||
    effectiveFirstToken.startsWith("../")
  ) return true;
  ...
  if (/^[A-Z]/.test(effectiveFirstToken) && tokens.length >= 4) return false;
  ...
  if (!/[A-Za-z0-9]/.test(effectiveFirstToken) && tokens.length >= 4) return false;
