...
test('ROADMAP DB→render→parse round-trip parity', async (t) => {
  ...
  // Assert slice count
  assert.strictEqual(parsedSlices.length, dbSlices.length, 'T1: slice count matches');

  // Assert field parity for each slice
  for (let i = 0; i < dbSlices.length; i++) {
    const db = dbSlices[i];
    const parsed = parsedSlices[i];
    assert.strictEqual(parsed.id, db.id, `T1: slice[${i}].id`);
    assert.strictEqual(parsed.title, db.title, `T1: slice[${i}].title`);
    assert.strictEqual(parsed.done, db.status === 'complete', `T1: slice[${i}].done matches status`);
    assert.strictEqual(parsed.risk, db.risk, `T1: slice[${i}].risk`);
    assert.strictEqual(JSON.stringify(parsed.depends), JSON.stringify(db.depends), `T1: slice[${i}].depends`);
  }
});
...
test('PLAN DB→render→parse round-trip parity', async (t) => {
  const base = createFixtureBase();
  const dbPath = join(base, '.gsd', 'gsd.db');
  openDatabase(dbPath);
  t.after(() => {
  ...
  // Assert task count
  assert.strictEqual(parsedPlan.tasks.length, 3, 'T2: task count matches');

  // Assert field parity for each task
  for (let i = 0; i < dbTasks.length; i++) {
    const db = dbTasks[i];
    const parsed = parsedPlan.tasks[i];
    assert.strictEqual(parsed.id, db.id, `T2: task[${i}].id`);
    assert.strictEqual(parsed.title, db.title, `T2: task[${i}].title`);
    assert.strictEqual(parsed.verify, db.verify, `T2: task[${i}].verify`);
    assert.strictEqual(parsed.done, db.status === 'complete', `T2: task[${i}].done matches status`);
  }
  ...
  assert.strictEqual(parsedPlan.tasks[0].id, 'T01', 'T2: first task is T01 (sequence 1)');
  assert.strictEqual(parsedPlan.tasks[1].id, 'T02', 'T2: second task is T02 (sequence 2)');
  assert.strictEqual(parsedPlan.tasks[2].id, 'T03', 'T2: third task is T03 (sequence 3)');
...
test('Sequence ordering parity — non-sequential insertion order', async (t) => {
  const base = createFixtureBase();
  const dbPath = join(base, '.gsd', 'gsd.db');
  openDatabase(dbPath);
  t.after(() => {
  ...
  assert.strictEqual(dbSlices.length, 4, 'T3: DB returns 4 slices');
  assert.strictEqual(dbSlices[0].id, 'S01', 'T3: DB first slice is S01 (sequence 1)');
  assert.strictEqual(dbSlices[1].id, 'S02', 'T3: DB second slice is S02 (sequence 2)');
  assert.strictEqual(dbSlices[2].id, 'S03', 'T3: DB third slice is S03 (sequence 3)');
  assert.strictEqual(dbSlices[3].id, 'S04', 'T3: DB fourth slice is S04 (sequence 4)');
  ...
  assert.strictEqual(parsedSlices.length, 4, 'T3: parsed 4 slices');
  assert.strictEqual(parsedSlices[0].id, 'S01', 'T3: parsed first slice is S01 (sequence 1)');
  assert.strictEqual(parsedSlices[1].id, 'S02', 'T3: parsed second slice is S02 (sequence 2)');
  assert.strictEqual(parsedSlices[2].id, 'S03', 'T3: parsed third slice is S03 (sequence 3)');
  assert.strictEqual(parsedSlices[3].id, 'S04', 'T3: parsed fourth slice is S04 (sequence 4)');

  // Assert full parity through DB→render→parse round-trip
  for (let i = 0; i < 4; i++) {
    assert.strictEqual(parsedSlices[i].id, dbSlices[i].id, `T3: round-trip slice[${i}].id`);
    assert.strictEqual(parsedSlices[i].done, dbSlices[i].status === 'complete', `T3: round-trip slice[${i}].done`);
    assert.strictEqual(parsedSlices[i].title, dbSlices[i].title, `T3: round-trip slice[${i}].title`);
  }
});
