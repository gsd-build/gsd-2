...
test('regex-hardening: all 12 sites accept both formats', async () => {
  ...
  //     Pattern: /^(M\d+(?:-[a-z0-9]{6})?)/
  {
    ...
    assert.strictEqual(('M001' as string).match(DIR_SCAN_RE)?.[1], 'M001', 'captures M001');
    ...
    assert.strictEqual(('M001-abc123' as string).match(DIR_SCAN_RE)?.[1], 'M001-abc123', 'captures M001-abc123 from dir name');
  ...
  //     Pattern: /^M\d+(?:-[a-z0-9]{6})?[^:]*:\s*/
  {
    ...
    assert.strictEqual('M001: Title'.replace(TITLE_STRIP_RE, ''), 'Title', 'strips M001: Title → Title');
    assert.strictEqual('M042: Payment Integration'.replace(TITLE_STRIP_RE, ''), 'Payment Integration', 'strips M042: Payment Integration');

    // Unique format strip
    assert.strictEqual('M001-abc123: Title'.replace(TITLE_STRIP_RE, ''), 'Title', 'strips M001-abc123: Title → Title');
    assert.strictEqual('M042-z9a8b7: Dashboard'.replace(TITLE_STRIP_RE, ''), 'Dashboard', 'strips M042-z9a8b7: Dashboard');
    ...
    assert.strictEqual('S01: Slice Title'.replace(TITLE_STRIP_RE, ''), 'S01: Slice Title', 'does not strip S01 prefix');
  }
  ...
  //     Pattern: /^gsd\/(?:([a-zA-Z0-9_-]+)\/)?(M\d+(?:-[a-z0-9]{6})?)\/(S\d+)$/
  {
    // Classic format — no worktree prefix
    {
      ...
      assert.strictEqual(m?.[1], undefined, 'no worktree prefix for gsd/M001/S01');
      assert.strictEqual(m?.[2], 'M001', 'captures M001');
      assert.strictEqual(m?.[3], 'S01', 'captures S01');
    }
    ...
    // Unique format — no worktree prefix
    {
      ...
      assert.strictEqual(m?.[1], undefined, 'no worktree prefix for unique format');
      assert.strictEqual(m?.[2], 'M001-abc123', 'captures M001-abc123');
      assert.strictEqual(m?.[3], 'S01', 'captures S01');
    }
    ...
    // Classic format — with worktree prefix
    {
      ...
      assert.strictEqual(m?.[1], 'worktree', 'captures worktree prefix');
      assert.strictEqual(m?.[2], 'M001', 'captures M001 with worktree');
      assert.strictEqual(m?.[3], 'S01', 'captures S01 with worktree');
    }
    ...
    // Unique format — with worktree prefix
    {
      ...
      assert.strictEqual(m?.[1], 'worktree', 'captures worktree prefix with unique format');
      assert.strictEqual(m?.[2], 'M001-abc123', 'captures M001-abc123 with worktree');
      assert.strictEqual(m?.[3], 'S01', 'captures S01 with worktree and unique format');
    }
  ...
  // (f) Prompt dispatch regexes — used in index.ts (executeMatch, resumeMatch)
  {
    ...
    // Execute — classic format
    {
      ...
      assert.strictEqual(m?.[1], 'T01', 'execute captures T01');
      assert.strictEqual(m?.[3], 'S01', 'execute captures S01');
      assert.strictEqual(m?.[4], 'M001', 'execute captures M001');
    }
    ...
    // Execute — unique format
    {
      ...
      assert.strictEqual(m?.[1], 'T02', 'execute captures T02 (unique format)');
      assert.strictEqual(m?.[3], 'S03', 'execute captures S03 (unique format)');
      assert.strictEqual(m?.[4], 'M001-abc123', 'execute captures M001-abc123');
    }
    ...
    // Resume — classic format
    {
      ...
      assert.strictEqual(m?.[1], 'S02', 'resume captures S02');
      assert.strictEqual(m?.[2], 'M001', 'resume captures M001');
    }
    ...
    // Resume — unique format
    {
      ...
      assert.strictEqual(m?.[1], 'S01', 'resume captures S01 (unique format)');
      assert.strictEqual(m?.[2], 'M042-z9a8b7', 'resume captures M042-z9a8b7');
    }
  }
  ...
  // (g) milestoneIdSort — mixed-format ordering
  {
    ...
    assert.strictEqual(sameSorted[0], 'M001-abc123', 'same seq preserves order (first)');
    assert.strictEqual(sameSorted[1], 'M001', 'same seq preserves order (second)');
  ...
  {
    // Classic format
    assert.strictEqual(extractMilestoneSeq('M001'), 1, 'M001 → 1');
    assert.strictEqual(extractMilestoneSeq('M042'), 42, 'M042 → 42');
    assert.strictEqual(extractMilestoneSeq('M999'), 999, 'M999 → 999');

    // Unique format — confirms dispatch-guard refactor correctness
    assert.strictEqual(extractMilestoneSeq('M001-abc123'), 1, 'M001-abc123 → 1');
    assert.strictEqual(extractMilestoneSeq('M042-z9a8b7'), 42, 'M042-z9a8b7 → 42');
    assert.strictEqual(extractMilestoneSeq('M100-xyz789'), 100, 'M100-xyz789 → 100');

    // Invalid → 0 (not NaN — the old parseInt(slice(1)) bug)
    assert.strictEqual(extractMilestoneSeq(''), 0, 'empty → 0');
    assert.strictEqual(extractMilestoneSeq('notes'), 0, 'notes → 0');
    assert.strictEqual(extractMilestoneSeq('S01'), 0, 'S01 → 0');
