...
describe("CmuxClient stdio isolation", () => {
  test("runSync and runAsync explicitly set stdio to prevent terminal interference", () => {
    // Read the cmux index source and verify that execFileSync/execFileAsync calls
  ...
  test("isCmuxCliAvailable uses stdio ignore to prevent terminal interference", () => {
    ...
    const source = fs.readFileSync(cmuxIndexPath, "utf-8");
    const fnMatch = source.match(/export function isCmuxCliAvailable\(\)[^{]*\{([\s\S]*?)\n\}/);
    assert.ok(fnMatch, "isCmuxCliAvailable function must exist");
    assert.ok(
      fnMatch[1].includes('stdio: "ignore"'),
      "isCmuxCliAvailable must use stdio: 'ignore' to prevent terminal interference",
    );
  });
});
