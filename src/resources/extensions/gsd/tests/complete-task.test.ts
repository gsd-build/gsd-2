...
test('complete-task: schema v5 migration', () => {
...
test('complete-task: accessor CRUD', () => {
  const dbPath = tempDbPath();
  openDatabase(dbPath);
...
test('complete-task: accessor stale-state error', () => {
...
test('complete-task: handler happy path', async () => {
  const dbPath = tempDbPath();
  openDatabase(dbPath);
...
test('complete-task: handler validation errors', async () => {
  const dbPath = tempDbPath();
  openDatabase(dbPath);
...
test('complete-task: handler idempotency', async () => {
  const dbPath = tempDbPath();
  openDatabase(dbPath);
  ...
  assert.strictEqual(tasks.length, 1, 'should have exactly 1 task row after first call');
  ...
  assert.strictEqual(tasksAfter.length, 1, 'should still have exactly 1 task row after rejected second call');
...
test('complete-task: handler with missing plan file', async () => {
  const dbPath = tempDbPath();
  openDatabase(dbPath);
