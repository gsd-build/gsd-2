...
test('#2330: Merge conflict stops auto loop', () => {
  // ── Test 1: worktree-resolver re-throws MergeConflictError ──────────────

  const methodStart = resolverSrc.indexOf("Worktree-mode merge:");
  assert.ok(methodStart > 0, "worktree-resolver has _mergeWorktreeMode method");
  ...
  assert.ok(instanceofIdx > 0, "auto/phases.ts has instanceof MergeConflictError check");
