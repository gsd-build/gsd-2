...
**Reviewer C — Assessment, Acceptance Criteria & KERN Validation**
Prompt: "Review milestone {{milestoneId}} assessment evidence, acceptance criteria, and KERN priority delivery. Working directory: {{workingDirectory}}. Read `.gsd/{{milestoneId}}/CONTEXT.md` for acceptance criteria and the Milestone Intent section. Check for ASSESSMENT files in each slice directory. Verify each acceptance criterion maps to either a passing assessment result or clear SUMMARY evidence. Then review the inlined milestone verification classes from planning. For each non-empty planned class, output a markdown table: Class | Planned Check | Evidence | Verdict. Use the exact class names `Contract`, `Integration`, `Operational`, and `UAT` whenever those classes are present. If no verification classes were planned, say that explicitly. **KERN Priority Validation:** If a Milestone Intent section exists in CONTEXT.md, verify that the KERN priority (the non-negotiable item from the Priority Stack) was actually delivered by at least one slice — check for concrete evidence in slice summaries, not just that a slice was planned for it. If the KERN was deferred, descoped, or only partially addressed, flag it explicitly. Check whether the Core Problem Being Eliminated was actually eliminated. Output three sections: `Acceptance Criteria` with a checklist `[ ] Criterion | Evidence`, `Verification Classes` with the table, and `KERN Priority` with delivery status. End with a one-line verdict: PASS if all criteria, verification classes, and KERN are covered, NEEDS-ATTENTION if gaps exist, FAIL if KERN was missed."
...
- **KERN override:** If Reviewer C reports that the KERN priority was missed or only partially delivered, the overall verdict cannot be `pass` without explicit justification in the rationale
