...
| `requiresThinkingAsText` | Convert thinking blocks to plain text before replay |
| `stripReasoningFromHistory` | Strip `reasoning_content`/`reasoning_details` from replayed messages (for TRT-LLM, vLLM) |
