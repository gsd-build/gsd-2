...
// Routes that must remain reachable while a shutdown is pending so the client
// can cancel the exit (boot) or confirm it (shutdown).
const SHUTDOWN_EXEMPT_PATHS = new Set(["/api/boot", "/api/shutdown"])
...
export function middleware(request: NextRequest): NextResponse | undefined {
  ...
  // Block new work while a shutdown is imminent, but let the boot and shutdown
  // routes through so the client can still cancel or confirm the exit.
  if (isShutdownPending() && !SHUTDOWN_EXEMPT_PATHS.has(pathname)) {
